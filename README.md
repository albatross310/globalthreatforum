...
included. Set `SUPABASE_URL` and `SUPABASE_ANON_KEY` in the host's
environment. The anon key is safe to expose publicly — all authorization is
enforced by Postgres RLS, not by secrecy of the key.

## Security model (for contributors)

- The server only ever uses the **anon key** with the requesting user's
  cookie-bound session — there is no service-role key in the app, so the app
